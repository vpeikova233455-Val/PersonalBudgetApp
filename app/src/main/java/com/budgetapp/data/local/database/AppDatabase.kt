...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE user_category_preferences ADD COLUMN syncStatus TEXT NOT NULL DEFAULT 'PENDING'"
                )
                database.execSQL(
                    "ALTER TABLE user_category_preferences ADD COLUMN lastModifiedTimestamp INTEGER NOT NULL DEFAULT 0"
                )
            }
        }
    }
