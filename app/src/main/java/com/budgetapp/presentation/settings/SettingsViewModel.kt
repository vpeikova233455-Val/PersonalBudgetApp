    ...
    private fun observeSyncStatus() {
        viewModelScope.launch {
            syncRepository.getSyncStatus().collect { syncStatus ->
                ...
                _uiState.update { it.copy(syncStatus = syncStatus, lastSyncFormatted = lastSyncFormatted) }
            }
        }
    }
    ...
    fun logout() {
        viewModelScope.launch {
            ...
            when (val result = authRepository.logout()) {
                is Result.Success -> _uiState.update { it.copy(isLoggingOut = false, logoutSuccess = true) }
                is Result.Error   -> _uiState.update { it.copy(isLoggingOut = false, logoutError = result.exception.message ?: "Logout failed") }
                is Result.Loading -> {}
            }
        }
    }
